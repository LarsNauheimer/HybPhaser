...
output_assess <- file.path(path_to_output_folder,paste("02_assessment", name_for_particular_dataset_optimization_subset, sep=""))
...
write.csv(tab_snps, file = file.path(output_assess,"0_Table_SNPs.csv"))
write.csv(tab_length, file = file.path(output_assess,"0_Table_consensus_length.csv"))

write.csv(tab_snps_cl2b,file=file.path(output_assess,"4_Table_SNPs_cleaned.csv"))
write.csv(tab_length_cl2b,file=file.path(output_assess,"4_Table_consensus_length_cleaned.csv"))

# txt file with included samples
write(samples[which(!(samples %in% outsamples_missing))], file=file.path(output_assess,"0_Samples_included.txt"))
...
for(i in 1:length(colnames(tab_snps_cl2b))){
  ...
  tab_het_ad$'loci with >0.25% SNPs'[i] <- 100*round(1 - length(which(tab_snps_cl2b[,i]<0.0025))/ (nloci_cl2-length(which(is.na(tab_snps_cl2b[,i])))),4)
