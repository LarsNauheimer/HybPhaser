...
sample <- gsub("(_mapped.*|.fastq.*|_bbsplit-stats.*)","",stats_files)  # this removes everything after the first . in the name
...
for(i in 1:length(rownames(tab_clade_assoc))){
  if(length(grep(ref_samples$sample[i],colnames(tab_clade_assoc)))>0){
    tab_clade_assoc_norm[i,] <- tab_clade_assoc[i,] / tab_clade_assoc[i,grep(ref_samples$sample[i],colnames(tab_clade_assoc))]
  }
}
...
write.csv(tab_hetad_cladeasso, file=file.path(path_to_clade_association_folder,"Table_combined_het_and-clade_association.csv"))
write.csv(tab_hetad_cladeasso_norm, file=file.path(path_to_clade_association_folder,"Table_combined_het_and-clade_association_normalised.csv"))
