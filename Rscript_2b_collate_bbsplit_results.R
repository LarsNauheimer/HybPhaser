...
if("abbreviation" %in% colnames(ref_samples)){
  ...
  ref_names <- ref_samples$samples
}


# generate empty table (ref_names x samples)
tab_clade_assoc <- matrix(nrow = length(ref_names), ncol = length(samples))
rownames(tab_clade_assoc) <- ref_names
colnames(tab_clade_assoc) <- samples
...
ttab_clade_assoc$samples <- rownames(ttab_clade_assoc)

ttab_clade_assoc_norm <- as.data.frame(t(tab_clade_assoc_norm))
ttab_clade_assoc_norm$samples <- rownames(ttab_clade_assoc_norm)
