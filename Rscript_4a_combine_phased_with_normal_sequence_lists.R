...
folder_seqlist_normal <- file.path(paste(path_for_HybPhaser_output,"/sequence_lists/",sequence_type, sep=""))
folder_seqlist_phased <-file.path(paste(path_for_HybPhaser_phased_output,"/sequence_lists/",sequence_type, sep=""))
folder_output <- file.path(paste(path_for_HybPhaser_phased_output,"/sequence_lists/",name_of_sequence_list_output, sep=""))
...
for(seqlp in seqlists_phased){
  ...
  if(loci_p %in% gsub(".*/(.*)_c.*","\\1",seqlists_normal)){
    file.append(seqlists_out_full[grep(loci_p, seqlists_out)], seqlp)  
  } else if(include_phased_seqlists_when_non_phased_locus_absent == "yes"){
...
if(file_with_samples_included == "") {
  samples_include_normal <- readLines(txt_file_with_list_of_accessions)
  samples_include_phased <- readLines(txt_file_with_list_of_phased_accessions)
