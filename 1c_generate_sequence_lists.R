...
output_Robjects <- file.path(path_to_output_folder,"00_R_objects", gsub("_","",name_for_dataset_optimization_subset))
...
dir.create(output_sequences, showWarnings = F, recursive = T)
dir.create(folder4seq_consensus_loci, showWarnings = F, recursive = T)
dir.create(folder4seq_consensus_samples, showWarnings = F, recursive = T)
dir.create(folder4seq_contig_loci, showWarnings = F, recursive = T)
dir.create(folder4seq_contig_samples, showWarnings = F, recursive = T)
...
if(length(dirs_not_in_sample_list) !=0 ){
  ...
  for(raw_consensus_file in list.files(folder4seq_consensus_loci, full.names = TRUE)){
    ...
    lines_with_samplename <- grep(paste(dirs_not_in_sample_list,collapse="|"),locus_consensus)
  ...
  for(raw_contig_file in list.files(folder4seq_contig_loci, full.names = TRUE)){
    ...
    lines_with_samplename <- grep(paste(dirs_not_in_sample_list,collapse="|"),locus_contig)
    if(length(lines_with_samplename) !=0){
      lines_to_remove <- c(lines_with_samplename,lines_with_samplename+1)
...
if(length(loci_to_remove)!=0){
  loci_files_to_remove_consensus <- loci_files_consensus[grep(paste(c(loci_to_remove),collapse="|"),loci_files_consensus)]
  loci_files_to_remove_contig <- loci_files_contig[grep(paste(c(loci_to_remove),collapse="|"),loci_files_contig)]
...
for(locus in rownames(tab_snps_cl2b)){
  ...
  if(length(samples_to_remove) !=0 ){
    ...
    lines_with_samplename <- grep(paste(samples_to_remove,collapse="|"),locus_consensus)
    if(length(lines_with_samplename) !=0){
      lines_to_remove <- c(lines_with_samplename,lines_with_samplename+1)
      locus_file_red <- locus_consensus[-lines_to_remove]
    ...
    lines_with_samplename <- grep(paste(samples_to_remove,collapse="|"),locus_contig)
    if(length(lines_with_samplename) !=0){
      lines_to_remove <- c(lines_with_samplename,lines_with_samplename+1)
      locus_file_hp_red <- locus_contig[-lines_to_remove]
...
if(length(outsamples_missing)==0){
  ...
  samples_files_to_remove_consensus <- sample_files_consensus[grep(paste(c(outsamples_missing),collapse="|"),sample_files_consensus)]
  samples_files_to_remove_contig <- sample_files_contig[grep(paste(c(outsamples_missing),collapse="|"),sample_files_contig)]
...
for(sample in samples_in){
  ...
  if(length(loci_to_remove)!=0){
    ...
    lines_with_lociname <- grep(paste(loci_to_remove,collapse="|"),samples_consensus)
    ...
    lines_with_lociname <- grep(paste(loci_to_remove,collapse="|"),samples_contig)
    if(length(lines_with_lociname) !=0){
      lines_to_remove <- c(lines_with_lociname,lines_with_lociname+1)
